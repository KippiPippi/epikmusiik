...
module.exports = {
  ...
  run: async (client, interaction) => {
    try {
      ...
      if (!name) return interaction.reply({ content: `❌ kehari?!?!?!`, ephemeral: true }).catch(e => { });
      let res;
      try {
        ...
        return interaction.editReply({ content: `❌ vamma:(` }).catch(e => { });
      }

      if (!res || !res.length || !res.length > 1) return interaction.reply({ content: `❌ vamma:(`, ephemeral: true }).catch(e => { });

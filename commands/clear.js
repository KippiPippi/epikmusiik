  ...
  run: async (client, interaction) => {
    ...
    try {
      if (!queue || !queue.playing) {
        return interaction.reply({ content: 'ei soi musiikis vamma', ephemeral: true });
      }

      if (!queue.songs[0]) {
        return interaction.reply({ content: 'ei musiik vamma olet', ephemeral: true });
      }
